...
def writePageToDB (propiedadesInfo):
    ...
    try:
        ...
        query = """ INSERT OR IGNORE INTO INMUEBLE (Propietario, Nombre, Direccion, Latitud, Longitud, Terreno, Construccion, NumHabitacion, NumSanitario,
                                            NumEstacionamiento, Descripcion, Precio) 
                    VALUES (?,?,?,?,?,?,?,?,?,?,?,?) """
        ...
        for row in data.index:
            ...
            if address != None:
                cursor.execute (query, ("century21.realestate@century21.com", l[0], address[0], address[1][0], address[1][1], int(l[3]), int(l[4]), int(l[5]), int(l[6]), int(l[7]), l[8], l[9]))
                conn.commit ()
